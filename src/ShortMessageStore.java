...
public class ShortMessageStore
{
    ...
    public synchronized String printmsg(String mobileNo, String msg)
    {
    	String mobilemsg = "false";
        if (messages.size() != 0) {
            ShortMessageValue sMV;
            Enumeration<String> keys = messages.keys();
            Object key;
            while (keys.hasMoreElements()) {
                key = keys.nextElement();
                sMV = (ShortMessageValue)messages.get(key);
                String receivemsg = printMobileMessage(mobileNo, key, sMV);
                if(receivemsg.equalsIgnoreCase(msg)) {
                	mobilemsg = "true";
                }
            }
    ...
    private String printMobileMessage(String mobileNo, Object key, ShortMessageValue sMV)
    {
        String serviceType, destAddr, shortMessage, message = "";
        ...
        if (mobileNo.equalsIgnoreCase(destAddr)){
