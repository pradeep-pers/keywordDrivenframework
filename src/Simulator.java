...
public class Simulator 
{
     ...
     */
    //static String usersFileName = FileOperations.getTemplateProjectFolder()+"//Resources//users.txt";
    ...
    private static SMSCListener smscListener = null;
    private static SimulatorPDUProcessorFactory factory = null;
    private static PDUProcessorGroup processors = null;
    private static ShortMessageStore messageStore = null;
    private static DeliveryInfoSender deliveryInfoSender = null;
    private static Table users = null;
    private static boolean displayInfo = true;
...
/*    protected void menu() throws IOException
     ...
     * just after.
     */
    protected static void start(String smport) throws IOException
    {
    	String usersFileName = FileOperations.getTemplateProjectFolder()+"//Resources//users.txt";
        if (smscListener == null) {
            //System.out.print("Enter port number> ");
            //int port = Integer.parseInt(keyboard.readLine());
            int port = Integer.parseInt(smport);
    ...
    protected static boolean verifymsg(String mobileNo,String msg)
    {
    	String verify = "false";
        if (smscListener != null) {
            verify = messageStore.printmsg(mobileNo, msg);
        } else {
            System.out.println("You must start listener first.");
        }
     ...
     * @throws IOException 
     */
    protected void reloadUsers() throws IOException
    {
    	String usersFileName = FileOperations.getTemplateProjectFolder()+"//Resources//users.txt";
        if (smscListener != null) {
            try {
     ...
     * @return 
     */
    protected static boolean listClients()
    {
     ...
     */
   /* protected static void sendMessage() throws IOException
    {
        if (smscListener != null) {
            int procCount = processors.count();
            if (procCount > 0) {
                ...
                for(int i=0; i<procCount; i++) {
                    ...
                    if (proc.getSystemId().equals(client)) {
                        if (proc.isActive()) {
                            System.out.print("Type subscriber id> ");
                            String subId = keyboard.readLine();
                            System.out.print("Type the message> ");
                            String message = keyboard.readLine();
                            DeliverSM request = new DeliverSM();
                            try {
                                request.setShortMessage(message);
                                request.setSourceAddr(subId);
    ...
    protected static void sendMessage(String subId,String message) throws IOException
    {
        if (smscListener != null) {
            int procCount = processors.count();
            if (procCount > 0) {
